...
function draw(size) {
  ...
  const s = size/32;
  const tiles = [
    [3,3,12,12,3.5,'#8887a0',0.55],
    [17,3,12,12,3.5,'#8887a0',0.75],
    [3,17,12,12,3.5,'#8887a0',0.35],
  ];
  for (let y=0;y<size;y++){
    for (let x=0;x<size;x++){
      ...
      if (bgA>0){ r=0x0a;g=0x0b;b=0x11;a=bgA; }
      ...
      const ca = cov(cd)*a;
      if (ca>0){ r=r*(1-ca)+0x8f*ca; g=g*(1-ca)+0x80*ca; b=b*(1-ca)+0xf3*ca; a=Math.max(a,ca); }
      const i=(y*size+x)*4;
