/* Generates Tessera PNG icons (192/512) — four rounded tiles, brass dot.
...
function draw(size) {
  ...
  const s = size/32;
  const tiles = [
    [3,3,12,12,3.5,'#8a8698',0.55],
    [17,3,12,12,3.5,'#8a8698',0.75],
    [3,17,12,12,3.5,'#8a8698',0.35],
  ];
  for (let y=0;y<size;y++){
    for (let x=0;x<size;x++){
      ...
      if (bgA>0){ r=0x11;g=0x11;b=0x14;a=bgA; }
      ...
      const ca = cov(cd)*a;
      if (ca>0){ r=r*(1-ca)+0xd9*ca; g=g*(1-ca)+0xa0*ca; b=b*(1-ca)+0x5b*ca; a=Math.max(a,ca); }
      const i=(y*size+x)*4;
